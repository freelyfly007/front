...
(function($){
	...
	function setSize(target, param){
		...
		t._size(opts, t.parent()).css('height', 'auto');
	...
	function bindEvents(target){
		...
		$(target).unbind('.calendar').bind('mouseover.calendar', function(e){
			...
			if (t.hasClass('calendar-menu-next') || t.hasClass('calendar-nextyear')){
				...
				if (menu.is(':visible')){
					menu.hide();
					$(target).find('.calendar-dtable').show();
				} else {
					showSelectMenus(target);
					$(target).find('.calendar-dtable').hide();
				}
	...
	function show(target){
		...
		if (saIndex >= 7) saIndex -= 7;		// 6
		if (suIndex >= 7) suIndex -= 7;		// 0
		...
		for(var i=0; i<weeks.length; i++){
			...
			for(var j=0; j<week.length; j++){
				...
				if (!(opts.year == day[0] && opts.month == day[1])){
					data.push('<td class="' + cls + ' calendar-disabled" abbr="'+s+'" style="' + styleValue + '"></td>');
				} else {
		...
		setRange(target, {max: opts.max, offsetMax: opts.offsetMax, min: opts.min, offsetMin: opts.offsetMin});

		opts.onNavigate.call(target, opts.year, opts.month);
		opts.onComplete.call(target);
	}
	
	// 设置选择范围的样式
	function setInRangeStyle(target, param) {
		var $td = $(target).find('.calendar-body .calendar-dtable td');
		$td.each(function() {
			var abbr = $(this).attr('abbr').replace(/[,]/g, '/');
			var tdDate = new Date(abbr).getTime();
			if (param.st && param.en) {
				if (tdDate >= param.st.getTime() && tdDate <= param.en.getTime()) {
	...
	// 设置日期范围
	function setRange(target, param) {
		param = param || {};
		var max = param.max || 0;
		var min = param.min || 0;
		var offsetMax = param.offsetMax || 0;
		var offsetMin = param.offsetMin || 0;
		
		if (!max && !min) {
			return;
		}
		
		var $calendarDay = $(target).find('.calendar-dtable td');
		max = _getTime(max, offsetMax);
		min = _getTime(min, offsetMin);
		$calendarDay.each(function() {
			var abbr = $(this).attr('abbr').replace(/,/g, '/');
			var abbrDate = Date.parse(abbr);
			
			if (max && abbrDate > max) {
				$(this).addClass('calendar-disabled');
			} else if (min && abbrDate < min) {
				$(this).addClass('calendar-disabled');
			}
		});
		
		function _getTime(date, offset) {
			...
			offset = isNaN(parseInt(offset)) ? 0 : parseInt(offset) * 24 * 60 * 60 * 1000;
			return date ? (date + offset) : 0;
		}
	}
	...
	$.fn.calendar.methods = {
		...
		getDateString: function(jq) {
			date = jq.find('.calendar-body .calendar-dtable td.calendar-selected').attr('abbr');
			return date;
		},
		setInRangeStyle: function(jq, param) {
			return jq.each(function() {
				setInRangeStyle(this, param);
			});
		}
	};
