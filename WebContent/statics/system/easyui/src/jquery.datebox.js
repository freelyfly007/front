...
(function($){
	...
	function createBox(target){
		...
		function setRange(target, param) {
			param = param || {};
			state.calendar.calendar('setRange', param);
		}
		...
		function setCalendar(target){
			...
			state.calendar.calendar(opts.range || {
				max: opts.max, 
				min: opts.min, 
	...
	$.fn.datebox.parseOptions = function(target){
		return $.extend({}, $.fn.combo.parseOptions(target), $.parser.parseOptions(target, ['sharedCalendar', 'max','min','offsetMax','offsetMin']));
	};
